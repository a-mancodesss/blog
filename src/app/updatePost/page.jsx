...
const UpdatePostPage = async ({ searchParams }) => {
  ...
  return (
    <div className="flex justify-center items-center py-10 min-h-[calc(100vh-200px)] relative overflow-hidden">
      ...
      <div className="w-full max-w-2xl bg-slate-900/50 backdrop-blur-md p-8 rounded-xl shadow-2xl border border-white/10 relative z-10">
        <h1 className="text-3xl font-bold mb-8 text-center text-white">Update Post</h1>
        <form action={updatePost} className="flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-2">
            <label htmlFor="imgUrl" className="text-slate-400 text-xs uppercase tracking-wider font-semibold ml-1">Cover Image (Optional)</label>
            <input 
              type="file" 
              name="imgUrl" 
              id="imgUrl" 
